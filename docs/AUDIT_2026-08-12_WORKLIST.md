...
| 04 | Three numbers went into `innerHTML` unescaped. They come from disk, and `POST /import` accepts `weekly/*.json` from any zip *(this row said `POST /restore` until 2026-08-30; the route has always been `/import` — the finding was right, the route name was wrong)* — so a restored backup could have run script in his dashboard | same | escaped; verified in a browser that a payload renders as text and injects zero nodes |
